 ...
 */
export async function encrypt(text: string, keyBase64: string): Promise<{ encrypted: string; iv: string }> {
 ...
 */
export async function decrypt(encryptedBase64: string, ivBase64: string, keyBase64: string): Promise<string> {
  const key = await importRawKey(keyBase64);
